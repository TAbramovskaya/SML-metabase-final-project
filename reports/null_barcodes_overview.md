...
We can assume that certain product categories are not eligible for discounts: in the data, there are no receipts where the discount code varies — it is either the same for all items in a receipt or equal to 0 (and for such transactions, the barcode is recorded as 'NULL'). Let’s take a look at one of these receipts as an example.
